...
#define POOL_SIZE                       (ES_ALIGN_UP(sizeof(struct dataBlock), sizeof(portReg_T)) * POOL_ELEMENTS)

int main(
    void) {

    static esSMemHandle_T myBigStorage;                     /* Static memory handle */
    ...
    esPMemInit(
        ...
        poolStorage,
        POOL_SIZE,
        ES_ALIGN_UP(sizeof(struct dataBlock), sizeof(portReg_T))); /* Initialize myPool pool memory */
