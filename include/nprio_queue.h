...
#define NPRIO_ARRAY_BUCKET_BITS                                                 \
    NLOG2_8(NDIVISION_ROUNDUP(CONFIG_BIAS_LEVELS, CONFIG_BIAS_BUCKETS))
...
struct nprio_queue
{
#if (CONFIG_BIAS_BUCKETS != 1)
    /**@brief       Priority bitmap structure
     * @notapi
     */
    struct nprio_bitmap
    {
#if   (CONFIG_BIAS_BUCKETS > NCPU_DATA_WIDTH) || defined(__DOXYGEN__)
        ncpu_reg                     group;                                     /**<@brief Bit group indicator        */
#endif  /* (CONFIG_BIAS_BUCKETS > NCPU_DATA_WIDTH) */
        ncpu_reg                     bit[NDIVISION_ROUNDUP(
                                         CONFIG_BIAS_BUCKETS,
                                         NCPU_DATA_WIDTH)];                     /**<@brief Bucket indicator           */
    }                           bitmap;                                         /**<@brief Priority bitmap            */
#endif  /* (CONFIG_BIAS_BUCKETS != 1) */
    struct nbias_list *         sentinel[CONFIG_BIAS_BUCKETS];
};
...
{
#if   (CONFIG_BIAS_BUCKETS > NCPU_DATA_WIDTH)
    uint_fast8_t                group;
...
#else   /*  (CONFIG_BIAS_BUCKETS > NCPU_DATA_WIDTH) */
    bitmap->bit[0] = 0u;
#endif  /* !(CONFIG_BIAS_BUCKETS > NCPU_DATA_WIDTH) */
}
